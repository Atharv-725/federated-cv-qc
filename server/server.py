...
class DPFedAvg(FedAvg):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        print(f"[Round {server_round}] epsilon={self.total_epsilon:.2f} | clients={len(results)} | failures={len(failures)}")
        return aggregated

    def aggregate_evaluate(self, server_round, results, failures):
        aggregated = super().aggregate_evaluate(server_round, results, failures)
        if results:
            avg_map = sum(r.metrics.get("mAP", 0) for _, r in results) / len(results)
            print(f"[Round {server_round}] avg mAP={avg_map:.4f}")
            for log in self.round_logs:
                if log["round"] == server_round:
                    log["avg_mAP"] = round(avg_map, 4)
            with open("server/logs/training_log.json", "w") as f:
                json.dump(self.round_logs, f, indent=2)
        return aggregated
...
print("Waiting for 3 factory clients to connect...")
